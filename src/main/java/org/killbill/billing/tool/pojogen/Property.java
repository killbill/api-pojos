...
public class Property {
  ...
  private static String identify(MethodUsage usage){
    ...
    if(isIsser(usage)){
      property = accessor.substring("is".length());
    } else if( isGetter(usage)) {
  ...
  private static boolean isGetter(MethodUsage usage, boolean is){
    ResolvedMethodDeclaration declaration = usage.getDeclaration();
    String name =  declaration.getName();
    Pattern pattern = Pattern.compile(is ? ISSER : GETTER);
  ...
  private static List<Property> sort(List<Property> properties ){
